...
def stopping_criteria(k, list_x, list_f, list_d, nb_iter, tol_rel, tol_abs):
    ...
    if abs(list_f[-1]) < tol_abs: return(True, "Racine localisée à {:7.1e} près : x_k = {:+14.7e} et f(x_k) = {:+11.4e}".format(tol_abs, list_x[-1], list_f[-1]))
    if list_d[-1] == 0:           return(True, "Dérivée exactement nulle au point courant x_k = {:+11.4e}".format(list_x[-1]))
    if k > 1:
...
def secante(f, x0, x1, nb_iter=100, tol_rel=10**-8, tol_abs=10**-8, output=""):
    ...
    Les arguments attendus sont :
        ...
        - deux scalaires x0 et x1 (de type int ou float), points de départ de la méthode itérative.
